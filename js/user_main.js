...
async function renderLibrary() {
    ...
    let localResults = allBooks.filter(b => {
        const matchSearch = b.name.toLowerCase().includes(searchInput) || 
    ...
    let googleResults = [];
    if (searchInput.length > 2 && currentLibCategory === 'all') {
        googleResults = await DB.searchGoogleBooks(searchInput);
    } 
    else if (currentLibCategory !== 'all' && localResults.length < 5) {
        googleResults = await DB.searchGoogleBooks(currentLibCategory);
    }
    ...
    if(booksToShow.length === 0) { 
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: #888; margin-top: 30px;">
            Không tìm thấy sách nào${currentLibCategory !== 'all' ? ` thuộc mục "${currentLibCategory}"` : ''}.
        </p>`; 
...
async function renderResources() {
    ...
    if(itemsToShow.length === 0) {
        grid.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: #888; padding: 30px;">Không tìm thấy tài liệu nào.</p>';
        document.getElementById('res-pagination').style.display = 'none';
        return;
    }
    ...
    itemsToShow.forEach(r => {
        ...
        let downloadBtnHtml = '';
        if (r.download_url) {
            downloadBtnHtml = `<button style="width:48%; background:#fff; color:#333; border:1px solid #ddd; padding:6px; border-radius:4px; cursor:pointer; font-size:12px;" onclick="window.open('${r.download_url}', '_blank')" title="Tải về máy"><i class="fas fa-download"></i> Tải</button>`;
        }
        ...
        grid.innerHTML += `
            <div class="book-item" style="display:flex; flex-direction:column; height: 100%;">
                ...
                <div style="height: 140px; background: #f9f9f9; display:flex; align-items:center; justify-content:center; border-bottom:1px solid #eee;">
                    ${displayImage}
                </div>

                <div class="book-info" style="flex:1; display:flex; flex-direction:column;">
                    <h4 title="${r.name}" style="font-size:14px; margin-bottom:5px; line-height: 1.4; max-height: 40px; overflow: hidden;">${r.name}</h4>
                    
                    <div style="margin-top:auto; display:flex; flex-wrap:wrap; gap:5px; align-items:center; margin-bottom:10px;">
                        <span class="${badgeClass}" style="font-size:10px;">${r.type}</span>
                        ${r.is_online ? '<span class="tag" style="font-size:10px; background:#fff7e6; color:#faad14; border:1px solid #ffe58f">Online</span>' : ''}
                        <span style="font-size:11px; color:#888; margin-left:auto;">${r.size || 'N/A'}</span>
                    </div>
...
function filterRes(type, el) {
    document.querySelectorAll('#res-filters .tag').forEach(t => t.classList.remove('active'));
...
function updatePaginationUI(prefix, totalPages, curPage) {
    ...
    let start = Math.max(1, curPage - 1);
    let end = Math.min(totalPages, curPage + 1);
...
function openDetail(id) {
    const book = currentLibFiltered.find(b => b.id == id); 
    if(!book) return;
...
function renderCardRegister() {
    ...
    if (myCard) {
        formContainer.style.display = 'none'; cardDisplay.style.display = 'flex';
...
async function handleReturn(loanId, bookId) { if(confirm("Xác nhận trả sách này?")) { if(await DB.returnBook(loanId, bookId)) { alert("Đã trả sách thành công!"); renderLoans(); loadStats(); } else alert("Lỗi khi trả sách!"); } }
...
function handleAvatarUpload(input) {
    if (input.files && input.files[0]) {
        const file = input.files[0]; if(file.size > 5 * 1024 * 1024) { alert("Ảnh quá lớn! Vui lòng chọn ảnh dưới 5MB."); input.value = ''; return; }
...
function performAvatarCrop() {
    ...
    if (canvas) {
        ...
        DB.updateUser(currentUser.id, { avatar_url: base64Img }).then(success => { if (success) { currentUser.avatar_url = base64Img; sessionStorage.setItem('currentUser', JSON.stringify(currentUser)); alert("✅ Đã cập nhật ảnh đại diện thành công!"); } else alert("Lỗi khi lưu ảnh lên hệ thống."); });
        closeAvatarModal();
    }
}
